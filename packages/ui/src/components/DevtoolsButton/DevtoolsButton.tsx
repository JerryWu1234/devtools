...
export const DevtoolsButton = component$(({ state }: DevtoolsButtonProps) => {
  ...
  return (
    <div
      ref={elementRef}
      class={{
        "fixed flex h-9 w-9 origin-center items-center justify-center rounded-lg border border-[var(--color-border-translucent)] bg-card backdrop-blur-md select-none": true,
        "rotate-90 border-[var(--color-glow-border)] bg-card bg-opacity-95 shadow-lg shadow-[var(--color-glow-shadow)]": state.isOpen.value && !isDragging.value,
